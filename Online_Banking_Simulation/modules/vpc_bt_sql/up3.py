...
def upload_to_bigtable(project_id, instance_id, table_id, input_file, batch_size=10):
    ...
    try:
        ...
        logger.info(f'Columns in the Parquet file: {df.columns.tolist()}')
    except Exception as e:
        logger.error(f'Failed to load Parquet file: {e}')
        return

    if 'Sub (User ID)' not in df.columns:
        logger.error("'Sub (User ID)' column not found in the Parquet file.")
        return
...
if __name__ == '__main__':
    ...
    input_file = '/home/kuppannan/cwt-pilot/Online_Banking_Simulation_Prj-1/modules/vpc_bt_sql/output_file.parquet'  # Update to your Parquet file path
